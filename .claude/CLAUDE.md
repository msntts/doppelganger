...
├── settings.json      # グローバル設定（permissions 等。hooks は doppelganger-core plugin で管理）
├── install.sh         # シンボリックリンク設定スクリプト
├── package.json       # hooks の TypeScript 依存関係
├── skills/            # カスタムスキル（~/.claude/skills/ の実体）
│   ├── doppelganger-core/  # コア hooks plugin（gatekeeper / observer / logger 等）
│   │   ├── .claude-plugin/plugin.json
│   │   └── hooks/hooks.json  # hook 登録（~/.claude/hooks/*.ts を参照）
│   ├── allow/         # 承認ポリシー注入スキル
...
各 hook の役割は `skills/doppelganger-core/hooks/hooks.json` と各 `.ts` の冒頭コメントを正とする。
hook の有効化は `claude plugin enable/disable doppelganger-core@skills-dir` で制御できる（`--scope project` でプロジェクト単位）。
