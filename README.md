...
1. **gatekeeper.ts (PreToolUse)** — 静的ルール（denied_patterns / allow_patterns / readonly_tools / git 操作）で即断。決着しなければ PermissionRequest に素通り。
2. **type: "prompt" hook (PermissionRequest, Haiku)** — 静的で決着しなかった操作を Haiku が yes/no 判定。不明なら fail-open（許可）。
