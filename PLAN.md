...
- [x] 5-1. `hooks/observer-prompt.ts:82` — `classifyResponse()` 呼び出しの直後に後処理ブランチを追加。`responseType` を `let` に変更し、`preceding?.skill === "review"` かつ `responseType === "unclear"` かつ `prompt.length < 20` かつ `!prompt.trimEnd().endsWith("？")` の場合に `responseType = "approval"` へ上書きする
